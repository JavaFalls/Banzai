...
#	var opponent_bot_ID = get_opponent(head.bot_ID)
	...
	fighter1.set_weapons(weapon_creator.create_weapon(weapon_creator.W_PRI_PRECISION_BOW), weapon_creator.create_weapon(weapon_creator.W_SEC_SCYTHE), weapon_creator.create_weapon(weapon_creator.W_ABI_SHIELD))
	get_node("fighter1_cooldowns").init(weapon_creator.W_PRI_PRECISION_BOW, fighter1.primary_weapon, weapon_creator.W_SEC_SCYTHE, fighter1.secondary_weapon, weapon_creator.W_ABI_SHIELD, fighter1.ability)
...
#	fighter1.get_node("animation_bot").load_colors_from_DB(head.bot_ID)
	...
	# TODO: Load weapons from DB or wherever else they might be handy to grap from
	fighter2.set_weapons(weapon_creator.create_weapon(weapon_creator.W_PRI_PRECISION_BOW), weapon_creator.create_weapon(weapon_creator.W_SEC_SCYTHE), weapon_creator.create_weapon(weapon_creator.W_ABI_SHIELD))
	get_node("fighter2_cooldowns").init(weapon_creator.W_PRI_PRECISION_BOW, fighter2.primary_weapon, weapon_creator.W_SEC_SCYTHE, fighter2.secondary_weapon, weapon_creator.W_ABI_SHIELD, fighter2.ability)
